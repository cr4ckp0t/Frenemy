...
local function GetDataObjectOptions()
	if not dataObjectOptions then
		...
		if LDBIcon then
			...
			dataObjectOptions.args.shortenLabels = {
				order = 2,
				type = "toggle",
				name = L.SHORTEN_LABELS,
				desc = L.SHORTEN_LABELS_DESC,
				get = function() return DB.DataObject.Labels.shorten end,
				set = function(info, value) DB.DataObject.Labels.shorten = not DB.DataObject.Labels.shorten end,
			}
		end
	end
