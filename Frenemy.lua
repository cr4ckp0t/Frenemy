...
local function UpdateStatistics()
	TotalBattleNetCount, OnlineBattleNetCount, _, FavoriteBattleNetCount = _G.BNGetNumFriends()
	...
	local function GenerateTooltipData()
		...
		if OnlineFriendsCount > 0 then
			for friendIndex = 1, OnlineFriendsCount do
				...
				local toonName, realmName = ("-"):split(friendInfo.name)

				WoWFriendIndexByName[friendInfo.name] = friendIndex
				WoWFriendIndexByName[toonName] = friendIndex

				if not OnlineFriendsByName[toonName] then
					local entry = {
						Class = friendInfo.className,
						FullToonName = friendInfo.name,
						IsLocalFriend = true,
						...
						ToonName = toonName,
						ZoneName = friendInfo.area ~= "" and friendInfo.area or _G.UNKNOWN,
					}
		...
		if TotalBattleNetCount > 0 then
			for battleNetIndex = 1, TotalBattleNetCount do
				local accountInfo = _G.C_BattleNet.GetFriendAccountInfo(battleNetIndex)
				local numToons = _G.C_BattleNet.GetFriendNumGameAccounts(battleNetIndex)

				for toonIndex = 1, numToons do
					local gameAccountInfo = _G.C_BattleNet.GetFriendGameAccountInfo(battleNetIndex, toonIndex)
					local characterName = _G.BNet_GetValidatedCharacterName(gameAccountInfo.characterName, accountInfo.battleTag, gameAccountInfo.clientProgram)
					local entry = {
						BroadcastText = (accountInfo.customMessage and accountInfo.customMessage ~= "") and ("%s%s%s (%s)|r"):format(BROADCAST_ICON, _G.FRIENDS_OTHER_NAME_COLOR_CODE, accountInfo.customMessage, _G.SecondsToTime(time() - accountInfo.customMessageTime, false, true, 1)) or nil,
						Class = gameAccountInfo.className,
						Client = gameAccountInfo.clientProgram,
						ClientIndex = CLIENT_SORT_ORDERS[gameAccountInfo.clientProgram] or 99,
						FactionIcon = FACTION_NAME_TO_ICON[gameAccountInfo.factionName],
						GameText = gameAccountInfo.richPresence ~= "" and gameAccountInfo.richPresence or _G.UNKNOWN,
						Level = gameAccountInfo.characterLevel and tonumber(gameAccountInfo.characterLevel) or 0,
						Note = accountInfo.note ~= "" and accountInfo.note,
						PresenceID = accountInfo.bnetAccountID,
						PresenceName = accountInfo.accountName or _G.UNKNOWN,
						RealmName = gameAccountInfo.realmName or "",
						StatusIcon = gameAccountInfo.isGameAFK and STATUS_ICON_AFK or (gameAccountInfo.isGameBusy and STATUS_ICON_DND or STATUS_ICON_ONLINE),
						ToonName = characterName,
						ZoneName = gameAccountInfo.areaName ~= "" and gameAccountInfo.areaName or _G.UNKNOWN,
					}

					if gameAccountInfo.clientProgram == _G.BNET_CLIENT_WOW then
						local existingFriend = OnlineFriendsByName[gameAccountInfo.characterName]

						if gameAccountInfo.realmName == PLAYER_REALM and existingFriend then
					...
					elseif not OnlineFriendsByPresenceName[entry.PresenceName] then
						if NON_GAME_CLIENT[gameAccountInfo.clientProgram] then
...
function DataObject:UpdateDisplay()
	local friendString = DB.DataObject.Labels.shorten and "F" or _G.FRIENDS
	local output = ("%s: %s%d/%d|r"):format(friendString, _G.BATTLENET_FONT_COLOR_CODE, OnlineFriendsCount + OnlineBattleNetCount, TotalFriendsCount + TotalBattleNetCount)

	if _G.IsInGuild() then
		local guildString = DB.DataObject.Labels.shorten and "G" or _G.GUILD
		output = ("%s %s: %s%d/%d|r"):format(output, guildString, _G.GREEN_FONT_COLOR_CODE, OnlineGuildMembersCount, TotalGuildMembersCount)
	end
